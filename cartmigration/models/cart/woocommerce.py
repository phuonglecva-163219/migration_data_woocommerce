...
class LeCartWoocommerce(LeCartWordpress):
	...
	def check_category_import(self, convert, category, categories_ext):
		...
		id_imported = self.get_map_field_by_src(self.TYPE_CATEGORY, convert['id'])
	...
	def before_category_import(self, convert, category, categories_ext):
		return response_success()
	def check_category_id_import(self, category_id):
		where = {
			'id_src': category_id
		}
	...
	def product_import(self, convert, product, products_ext):
		...
		guid = 'http://localhost/?post_type=product&#038;p={}'.format(post_id)
		data = {
			'guid': guid
		}
		where_id = {
		...
		for meta_key in post_meta:
			query = self.create_insert_query_connector("postmeta", {
				'post_id':post_id,
				'meta_key': meta_key,
				'meta_value': post_meta[meta_key]
			})
			post_meta_id = self.import_data_connector(query, 'postmeta')
	...
	def customer_import(self, convert, customer, customers_ext):
		user = {
			...
			'user_url':'',
			'user_registered':'2021-01-25 07:13:45',
			'user_activation_key':'',
			'user_status':'0',
			'display_name':convert['username']
		}

		insert_user_query = self.create_insert_query_connector("users", user)
		user_id = self.import_data_connector(insert_user_query, 'user')
		self.insert_map(self.TYPE_CUSTOMER, convert['id'], user_id)
		user_meta = {
			'show_admin_bar_front':'true',
			'wp_capabilities': 'a:1:{s:8:"customer";b:1;}',
			'billing_first_name':convert['first_name'],
			'billing_last_name':convert['last_name'],
			'billing_company':'LitEx',
			'billing_address_1':'dn',
			'billing_address_2':'hn',
			'billing_city':'12',
			'billing_postcode':'1000',
			'billing_country':'vn',
			'billing_state':'',
			'billing_phone':'9000-990-999',
			'billing_email':convert['email'],
			'shipping_first_name':convert['first_name'],
			'shipping_last_name':convert['last_name'],
			'shipping_company':'LitEx',
			'shipping_address_1':'dn',
			'shipping_address_2':'hn',
			'shipping_city':'12',
			'shipping_postcode':'9000',
			'shipping_country':'vn',
			'shipping_state':'',
		...
		for x, y in user_meta.items():
			qr = self.create_insert_query_connector("usermeta", {
				'user_id':user_id,
				'meta_key': x,
				'meta_value':y
			})
	...
	def order_import(self, convert, order, orders_ext):
		...
		for x, y in order_meta.items():
			data  = {
				'post_id': order_id,
				'meta_key': x,
				'meta_value': y
			}
			qr = self.create_insert_query_connector("postmeta", data)
			self.import_data_connector(qr, 'order_meta')
			woocommerce_item_data = {
				'_product_id': '',
				'_variation_id': '',
				'_qty': '',
				'_tax_class': '',
		...
		for item in convert['items']:
			...
			qr = self.create_insert_query_connector("woocommerce_order_items", order_item)
			self.import_data_connector(qr, 'woocommerce_order_items')
		#
		# self.insert_map(self.TYPE_ORDER, convert['id'], order['orders_id'], convert['code'])

		order_item_meta = {
			'_variation_id':'0',
			'_qty':'1',
			'_tax_class':'',
			'_line_subtotal':'10000',
			'_line_subtotal_tax':'0',
			'_line_total':'10000',
			'_line_tax':'0',
			'_line_tax_data':'a:2:{s:8:"subtotal";a:0:{}s:5:"total";a:0:{}}',
		}
		# for key, value in order_item_meta.items():
		# 	data = {
		# 		'order_item_id': order_id,
		# 		'meta_key': key,
		# 		'meta_value':value
		# 	}
		# 	qr = self.create_insert_query_connector("woocommerce_order_itemmeta", data)
		# 	self.import_data_connector(qr, 'wp_woocommerce_order_itemmeta')
		data = {
			'order_id':order_id,
			'parent_id':'0',
