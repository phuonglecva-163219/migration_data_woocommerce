...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		language_ids = duplicate_field_value_from_list(categories['data'], 'language_id')
		# print(category_ids)
		# print(language_ids)
	...
	def convert_category_export(self, category, categories_ext):
		...
		# category_data['created_at'] = datetime.fromtimestamp(to_int(category['date_added'])) if category['date_added'] else '0000-00-00 00:00:00'
		category_data['created_at'] = category['date_added']
		# category_data['updated_at'] = datetime.fromtimestamp(to_int(category['last_modified'])) if category['last_modified'] else '0000-00-00 00:00:00'
		...
		# 	print("{}: {}".format(x, category_data[x]))
		# print("=======================================================================================")
	...
	def get_products_ext_export(self, products):
		...
		# print("product_id_conn: {}".format(product_id_con))
		...
		if (not product_ext) or product_ext['result'] != 'success':
			return response_error()
		# Return here now
		# print("_________________extra")
		# print(product_ext)
		return product_ext

		image_ids=duplicate_field_value_from_list(product_ext['data']['product_Image'], 'image_id')
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['sku'] = product['products_quantity_tx']
		# print("status: {}".format(product['products_status']))
		...
		product_data['created_at'] = get_current_time()
		product_data['updated_at'] = product_data['created_at']
		# product_data['name'] = html.unescape(products_ext_data['products_description']['products_name'])
		product_data['name'] = get_row_from_list_by_field(products_ext_data['products_description'], 'products_id', product['products_id'])['products_name']
		# product_data['name'] = 'Product Sample Name'
		# product_data['description'] = html.unescape(product['descrip'])
		# product_data['description'] = products_ext_data['products_description']['products_description']
		product_data['description'] = get_row_from_list_by_field(products_ext_data['products_description'], 'products_id', product['products_id'])['products_description']
		product_description = None
		if 'products_meta' in products_ext_data:
			product_description = get_list_from_list_by_field(products_ext_data['products_meta'], 'product_id',product['products_id'])
		...
		# product_images = get_list_from_list_by_field(products_ext_data['products_images'], 'product_id',product['products_id'])
		product_images = get_list_from_list_by_field(product['products_image'], 'products_id',product['products_id'])
		if product_images:
			if check_thumbnail:
				for product_image in product_images:
					...
					if image_data :
						...
						product_image_data['url'] = "https://images.pexels.com/photos/221185/pexels-photo-221185.jpeg"
	...
	def get_customers_ext_export(self, customers):
		...
		# print("zone_ids: {}".format(zone_ids))
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		customer_data['username'] = customer['customers_firstname']
		# customer_data['email'] = customer['customers_email_address']
		customer_data['email'] = customer['customers_firstname'] + "@gmail.com"
		...
		if address_books:
			for address_book in address_books:
				...
				# print("address_data :")
				# print(address_data)
				# print(customers_ext['data']['countries'])
				...
				if state_id:
					state = get_row_from_list_by_field(customers_ext['data']['states'], 'zone_code', state_id)
					if state:
	...
	def get_orders_ext_export(self, orders):
		...
		orders_ext_queries = {
			...
			'currencies': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_currencies WHERE currencies_id=1" 
			}
		}
	...
	def convert_order_export(self, order, orders_ext):
		print("+++++++++++++++++++++++++++++++++++")
		print(orders_ext['data']['orders_products'])
		print(orders_ext['data']['orders_total'])
		print("+++++++++++++++++++++++++++++++++++")
		orders_ext_data = orders_ext['data']
		order_rows_id = get_list_from_list_by_list_field(orders_ext_data['orders_total'], 'orders_id', order['orders_id'])
		ot_subtotal = get_row_from_list_by_field(order_rows_id, 'class', 'ot_subtotal')
		ot_shipping = get_row_from_list_by_field(order_rows_id, 'class', 'ot_shipping')
		ot_total = get_row_from_list_by_field(order_rows_id, 'class', 'ot_total')
		ot_tax = get_row_from_list_by_field(order_rows_id, 'class', 'ot_tax')
		...
		shipping_details = [x for x in data if x['class'] == 'ot_shipping'][0]
		total_details = [x for x in data if x['class'] == 'ot_total'][0]
		# tax_details = [x for x in data if x['class'] == 'ot_tax'][0]
		subtotal_details = [x for x in data if x['class'] == 'ot_subtotal'][0]
		...
		order_data['tax']['amount'] = get_value_by_key_in_dict(orders_ext['data']['orders_products'], 'products_tax', 0.0000)
		order_data['shipping']['title'] = shipping_details['title'] if shipping_details['title'] else 'Shipping'
		order_data['shipping']['amount'] = shipping_details['value']
		order_data['discount']['title'] = 'Discount'
		order_data['discount']['amount'] = 0.0000
		order_data['total']['title'] = total_details['title'] if total_details['title'] else  'Total'
		order_data['total']['amount'] = total_details['value']
		order_data['subtotal']['title'] = subtotal_details['title'] if subtotal_details['title'] else 'Sub Total'
		order_data['subtotal']['amount'] = subtotal_details['value']
		...
		# print(orders_ext['data']['currencies'][0]['code'])
		# order_data['currency'] = orders_ext['data']['currencies'][0]['code']
		# order_data['currency_value'] = orders_ext['data']['currencies'][0]['value']
		# order_data['currency_value'] = orders_ext['data']['currencies'][0]['value']
		# order_data['created_at'] = orders_ext['data']['currencies']['code']
		# order_data['updated_at'] = orders_ext['data']['currencies'][0]['last_updated']

		order_customer = self.construct_order_customer()
		#order_customer = self.add_c(order_customer)
		order_customer['customers_id'] = order['client_customers_id']
		order_customer['email'] = order['customers_email']
		# customer_name = order['ship_fname'] + ' ' + order['ship_lname']
		customer_name = order['billing_name']
		order_customer['first_name'] = customer_name.split(" ")[0]
		order_customer['last_name'] = customer_name.split(" ")[1]
...
# 
		...
		if customer_country:
			...
			customer_address['country']['country_code'] = customer_country['countries_iso_code_3']
		...
		if billing_country:
			...
			order_billing['country']['code'] = billing_country['countries_id']
		...
		delivery_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_name', order['billing_country'])
		if delivery_country:
			order_delivery['country']['id'] = delivery_country['countries_id']
			order_delivery['country']['code'] = delivery_country['countries_id']
		...
		for order_product in order_products:
			order_item_subtotal = to_decimal(order_product['final_price']) * to_decimal(order_product['products_quantity'])
